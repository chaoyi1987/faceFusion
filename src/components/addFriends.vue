<template>
  <div id="addFriends">
    ...
    <section class="friends" style="vertical-align: middle;">
      ...
      <div class="apply" v-for="(item,index) in addInfoData" @touchstart="touchstart(index)">
        <router-link to="/home">
          <div class="wechat" style="margin-top: 0;">
            <img :src="item.weChat" alt="">
            <span>微信</span>
          </div>
          <div class="add_info">
            <img :src="item.img" alt="" />
            <div>
              <p><span style="width:1.7rem;display: inline-block;text-align:justify;">{{item.name}}</span>：您好，我是{{item.name}}</p>
